...
export function Header({ onQuote, home = false }: { onQuote: () => void; home?: boolean }) {
  ...
  return <header className={`site-header ${home?'':'detail-header'}`}>
    ...
    <nav aria-label="Main navigation"><a href="/#equipment">Equipment <span>⌄</span></a><a href="/#brands">Our brands</a><a href="/#finder">Equipment finder</a><a href="/#contact">Contact</a></nav>
    ...
    {menu&&<nav className="mobile-nav" id="mobile-nav" aria-label="Mobile navigation">{[['Equipment','equipment'],['Our brands','brands'],['Equipment finder','finder'],['Contact','contact']].map(([name,id])=><a key={id} href={`/#${id}`} onClick={()=>setMenu(false)}>{name}<ArrowUpRight size={18}/></a>)}</nav>}
  </header>;
}
